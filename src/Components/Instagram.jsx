...
const Instagram = () => {
    ...
    const cardWidth = 446; // প্রতিটি কার্ডের width + gap
    const visibleCards = 3; // একসাথে কতটি কার্ড দেখা যাবে
    const totalCards = 6; // মোট কার্ড সংখ্যা

    const containerWidth = cardWidth * visibleCards;
    const lastOffset = -(cardWidth * totalCards - containerWidth);

    const handlePrev = () => {
        setOffset(prev => Math.min(prev + cardWidth, 0)); // একবারে ১টি কার্ড left
    };

    const handleNext = () => {
        setOffset(prev => Math.max(prev - cardWidth, lastOffset)); // একবারে ১টি কার্ড right
    };
    ...
    return (
        <div className="bg-white">
            <div className="px-10 py-20 flex items-start justify-between mb-8 overflow-hidden mt-16 gap-15">
                {/* Left Text Section */}
